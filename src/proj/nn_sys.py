...
def gen_data(N):
    ...
    # r = np.vstack((np.linalg.norm(X, axis=1), np.linalg.norm(X, axis=1))).T
    # thetas = np.arctan2(X[:, 1], X[:, 0]) + np.pi / 2 - 0.0 * (np.linalg.norm(X, axis=1) - 1)
...
class UltraSimpleNN(nn.Module):
    ...
    def forward(self, x):
        # print(x.T[:3, :].T)
        return self.nonlin(self.layer1(x)) - x
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=250, gamma=0.1, verbose=False)
# Training loop
num_epochs = 1000
batch_size = 4
